...
##R
library(ggplot2)
a <- read.table("ribo-tisnum.txt",header = T) ##ribo-tisnum.txt in data folder
...
```R
library(ggplot2)
lib = read.table("lib1.txt",sep="\t",header=T) ##lib1.txt was in data folder, and stored the interaction numbers generated by mating 1-10 in lib1
